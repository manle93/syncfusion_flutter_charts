...
class SfCartesianChartState extends State<SfCartesianChart>
  ...
  ///
  /// If the parent widget rebuilds and request that this location in the tree update to display a new
  /// widget with the same `runtimeType` and `Widget.key`, the framework will update the widget
  /// property of this `State` object to refer to the new widget and then call this method with
  /// the previous widget as an argument.
  ///
  /// The framework always calls `build` after calling `didUpdateWidget`, which means any calls
  ///  to `setState` in `didUpdateWidget` are redundant.
  ///
  /// * In `didUpdateWidget` unsubscribe from the old object and subscribe to the new one if the
  ...
  /// and the `mounted` property is false. It is an error to call `setState` at this
  /// point. This stage of the lifecycle is terminal: there is no way to remount a
  /// `State` object that has been disposed.
