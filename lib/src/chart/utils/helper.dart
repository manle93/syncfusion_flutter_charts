    ...
    if (seriesRenderer._visible &&
      ...
      if (xValues.length == 1) {
        ...
        if (axisRenderer is DateTimeAxisRenderer) {
          minDate =
              DateTime.fromMillisecondsSinceEpoch(seriesRenderer._minimumX);
...
num dateTimeInterval(CartesianSeriesRenderer seriesRenderer) {
  final DateTimeAxis xAxis = seriesRenderer._xAxisRenderer._axis;
...
List<double> _calculateControlPoints(List<num> xValues, List<num> yValues,
  ...
  final List<double> controlPoints = List<double>.filled(4, null);
...
int _getVisibleDataPointIndex(
  ...
  int index;
  if (pointIndex > seriesRender._dataPoints.length) {
